...
export function AikaLogo({ href = "/", compact = false, className }: { href?: string; compact?: boolean; className?: string }) {
  const content = (
    <span className={cn("inline-flex items-center font-bold tracking-normal", className)} aria-label="AIKA">
      <span className="grid h-9 min-w-[4.5rem] place-items-center rounded-md border border-primary/50 bg-background px-3 text-base font-black text-primary shadow-glow sm:h-10 sm:min-w-20 sm:px-4 sm:text-lg">
        AIKA
      </span>
